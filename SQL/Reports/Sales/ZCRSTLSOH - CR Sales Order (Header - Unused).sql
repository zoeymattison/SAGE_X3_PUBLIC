...
PrepaymentAuthInfo AS (
    SELECT
        SEU.VCRNUM_0 AS NUM_0,
        SEU.ACCL4D_0,
        SUM(SEU.PAYAMT_0) AS PAYAMT_0,
        SUM(SEU.AUTAMT_0) AS AUTAMT_0,
        SUM(SEU.TAXAMT_0) AS TAXAMT_0,
        MAX(APL.LANMES_0) AS LANMES_0,
        MAX(SEU.ACCNCKNAM_0) AS ACCNCKNAM_0,
        MAX(SEU.STAFLG_0) AS STAFLG_0,
        MAX(ATX.TEXTE_0) AS TEXTE_0
    FROM
        LIVE.SEAUTH SEU
    LEFT JOIN
    ...
    LEFT JOIN
        ...
        AND ATX.IDENT1_0 = '398'
    WHERE
        SEU.VCRNUM_0 IN (SELECT PYD.NUM_0 FROM LIVE.GACCDUDATE DUD INNER JOIN LIVE.PAYMENTD PYD ON DUD.NUM_0 = PYD.VCRNUM_0 AND DUD.DUDLIG_0 = PYD.DUDLIG_0 WHERE DUD.PAMTYP_0 = 2)
        AND SEU.STAFLG_0 IN (3, 6)
    GROUP BY
        SEU.VCRNUM_0,
        SEU.ACCL4D_0
),
PrepayemntInfo AS (
    SELECT DISTINCT
        ...
        PAI.ACCL4D_0,
        PAI.PAYAMT_0 AS SEU_PAYAMT_0,
        PAI.AUTAMT_0 AS SEU_AUTAMT_0,
        PAI.TAXAMT_0 AS SEU_TAXAMT_0,
        PAI.LANMES_0,
        PAI.ACCNCKNAM_0,
        PAI.STAFLG_0,
        PAI.TEXTE_0
    FROM
        LIVE.GACCDUDATE DUD
    LEFT JOIN
        LIVE.PAYMENTD PYD ON DUD.NUM_0 = PYD.VCRNUM_0 AND DUD.DUDLIG_0=PYD.DUDLIG_0
    LEFT JOIN
        PrepaymentAuthInfo PAI ON PYD.NUM_0 = PAI.NUM_0
    WHERE
        DUD.PAMTYP_0 = 2
    GROUP BY
        DUD.NUM_0,
...
RankedPrepaymentInfo AS (
    SELECT
        ...
        PAYCUR_0,
        ACCL4D_0,
        SEU_PAYAMT_0 AS PAYAMT_0,
        SEU_AUTAMT_0 AS AUTAMT_0,
        SEU_TAXAMT_0 AS TAXAMT_0,
        LANMES_0,
        ACCNCKNAM_0,
        TEXTE_0,
...
SELECT
    ...
    END AS [Auth Amount],
    CASE
		WHEN SOH.PTE_0 = 'CREDITCARDP' AND CRD.LANMES_0='Authorized' THEN isnull(CRD.AUTAMT_0, 0)
        WHEN SOH.PTE_0 = 'CREDITCARDP' AND CRD.LANMES_0='Captured' THEN isnull(CRD.PAYAMT_0, 0)
