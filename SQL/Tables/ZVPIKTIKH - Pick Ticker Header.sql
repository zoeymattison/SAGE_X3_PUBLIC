with picktick_header as (
	select
		...
		isnull(t1.TEXTE_0,'') as [Header Text],
		isnull(t2.TEXTE_0,'') as [Footer Text],
		s.ORDDAT_0 as [Order Date]

	from
		LIVE.STOPREH h
	inner join
		LIVE.STOPRED d on h.PRHNUM_0=d.PRHNUM_0 and d.PRELIN_0=1
	inner join
...
where
	p.[Bill-to]='77779'
order by
	p.[Creation Date] desc, p.[Pick Ticket] desc