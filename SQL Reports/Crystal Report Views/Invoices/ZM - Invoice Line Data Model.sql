...
invoicelines as (
	select
		...
		(sid.DISCRGVAL1_0/100) as [Invoice Line Discount],
...
salesorderlines as (
	select
		...
		itm.ITMDES1_0+' '+itm.ITMDES2_0 as [Sales Order Description],
		...
		(sop.DISCRGVAL1_0/100) as [Sales Order Discount],
...
SELECT DISTINCT
	...
	case
		when son.SOHNUM_0='' then ivl.[Invoice Gross Price]
		when son.SOHNUM_0 is null then interest.[Amount]
		else sol.[Sales Order Gross Price]
	end as [Gross Price],
	isnull(case son.SOHNUM_0
		when '' then ivl.[Invoice Line Discount]
		else sol.[Sales Order Discount]
	end,0) as [Discount Percent],
	case
		when son.SOHNUM_0='' then ivl.[Invoice Net Price]
		when son.SOHNUM_0 is null then interest.[Amount]
		else sol.[Sales Order Net Price]
	end as [Net Price],
	case
